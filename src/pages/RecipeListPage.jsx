    ...
    <Center flexDir="column" gap={'40px'} alignContent={'center'} backgroundColor={'gray.100'}>
      ...
      <Input placeholder='Look up a recipe...' maxW={'400px'} width={'95vw'} backgroundColor={'white'} onInput={handleInput}/>
        ...
        <For each={!searchResults.length > 0 ? data.hits : searchResults}>
            {(item, index) => (
              <Card.Root width={'320px'} key={index} overflow={'hidden'} maxW={'sm'} height={'500px'} cursor={'pointer'} onClick={() => clickFn(item.recipe)} borderRadius={'15px'} boxShadow={'lg'}>
                <Image src={item.recipe.image} height={'200px'}/>
                <Card.Body gap={2} alignItems='center'>
                  <Text textTransform='uppercase'>{item.recipe.mealType}</Text>
                  <Card.Title textAlign={'center'}>{item.recipe.label}</Card.Title>
                  <Flex gap={1}>
                  ...
                  <Flex direction={'row'} gap={1} wrap={'wrap'} maxW={'100%'}>
